    ...
    >
      <Space direction="vertical" size={8} style={{ width: '100%' }}>
        ...
        {column === 'available' ? (
          ...
          <Button size="small" danger onClick={() => onRevoke(item.id)}>
            Thu hồi
          </Button>
        )}
      </Space>
...
function DelegationPage() {
  ...
  useEffect(() => {
    const loadDelegations = async () => {
      ...
      try {
        ...
        message.error(
          requestError.response?.data?.message || 'Không tải được nhật ký ủy quyền của user đã chọn.',
        );
      } finally {
  ...
  const ownDelegations = useMemo(
    () => selectedUserDelegations.filter((item) => item.grantor_user_id === meta.grantor?.user_id),
  ...
  const handleDelegate = async (permissionId) => {
    ...
    try {
      ...
      setSelectedUserDelegations((current) => [response.data.item, ...current]);
  ...
  const handleRevoke = async (delegationId) => {
    ...
    try {
      ...
      setSelectedUserDelegations((current) => current.filter((item) => item.id !== delegationId));
    ...
    <Space direction="vertical" size={20} style={{ width: '100%' }}>
      <Card className="page-card" styles={{ body: { padding: 28 } }}>
        ...
        <Typography.Paragraph className="page-subtitle">
          Vai trò cấp trên chỉ cấp quyền cho đúng user cần dùng, không cấp tràn cho cả role. Mỗi
          lần ủy quyền đều được lưu lại ai cấp, cấp quyền gì, cho user nào và có thể thu hồi riêng.
        ...
        {selectedTargetUser ? (
          <Space direction="vertical" size={18} style={{ width: '100%' }}>
            <div>
              ...
              <Typography.Paragraph type="secondary" style={{ marginBottom: 0 }}>
                Bạn đang thao tác cho <strong>{selectedTargetUser.full_name}</strong> (
                {selectedTargetUser.username}). Kéo quyền từ cột trái sang cột phải để cấp riêng cho
                user này, hoặc kéo ngược lại để thu hồi.
              </Typography.Paragraph>
            </div>
            ...
            <div className="delegation-grid">
              ...
              >
                ...
                <Typography.Text type="secondary">
                  Đây là các quyền do chính bạn cấp riêng cho user đang chọn.
                </Typography.Text>
                <div className="permission-list">
                  {ownDelegations.length ? ownDelegations.map((item) => (
                    ...
                    <Empty
                      image={Empty.PRESENTED_IMAGE_SIMPLE}
                      description="Bạn chưa cấp quyền riêng nào cho user này."
                    />
                  )}
                </div>
              </section>
      ...
      <Card className="page-card" styles={{ body: { padding: 28 } }}>
        <Typography.Title level={4} style={{ marginTop: 0 }}>
          Nhật ký ủy quyền của user đã chọn
        </Typography.Title>
        <Typography.Paragraph type="secondary">
          {selectedTargetUser
            ? `Dùng để truy vết ai đã cấp quyền nào cho ${selectedTargetUser.full_name}.`
            : 'Chọn một user để xem lịch sử ủy quyền chi tiết.'}
        </Typography.Paragraph>
        <Table
          rowKey="id"
          ...
          columns={[
            ...
            {
              title: 'Người cấp',
              dataIndex: 'grantor_user_name',
              key: 'grantor_user_name',
            },
            {
              title: 'Vai trò cấp',
              dataIndex: 'grantor_role_name',
              key: 'grantor_role_name',
            },
            {
              title: 'Người nhận',
